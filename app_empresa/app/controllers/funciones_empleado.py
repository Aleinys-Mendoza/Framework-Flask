    ...
    try:
        with connectionBD() as conexion_MySQLdb:
            with conexion_MySQLdb.cursor(dictionary=True) as cursor:
                ...
                Valores = (dataForm ['nombre_empleado'], dataForm['apellidos empleado'], dataForm['tipo_identidad'],
                            dataForm['n_identidad'], dataForm['focha_nacimiento'], dataForm['sexo'], 
...
def procesar_imagen_perfil(foto):
    try:
        ...
        #Creacion un string de 50 caracteres
...
#4 FUNCION : Detalles de empleado
...
#5 FUNCION: Funcion Empleados Infome(Reportes)
def empleadosReportes():
    try:
        with connectionBD as conexion_MySQLdb:
            with conexion_MySQLdb.cursor(dictionary=True) as cursor:
                querySQL = ("""
...
def generarReporteExcel():
    dataEmpleados = empleadosReportes()
    ...
    #Formato para numeros en moneda colombiana y sin decimales
    ...
    hoja.append((nombre_empleado,apellidos_empleado,tipo_identidad,n_identidad,fecha_nacimiento,sexo,grupo_rh,
                email,telefono,profesion,salario, fecha_registro))
    
    #Itera a travez de la firla y aplica el formato a la columna G
            ...
            with conexion_MySQLdb.cursor(dictionary=True) as mycursor:
                querySQL = ("""
                        SELECT 
                            ...
                            ORDER BY e.id_empleado DESC
                            """)
                search_pattern =f"%{search}%" #Agergar "%" alrededor del termino de busquedad
                mycursor.execute(querySQL,(search.pattern,))
                resultado_busqueda=mycursor.fetchall()
                return resultado_busqueda
            
    except Exception as e:
        print(f"Ocurrio un error en def buscarEmpleadosBD: {e}")
        return[]
...
def buscarEmpleadoUnico(id):
    try:
        ...
        print(f"Ocurrio un error en def buscarEmpleadoUnico: {e}")
        return[]

# 9 FUNCION: funciono que actualiza un empleado
...
# 10 FUNCION: funcion lista de usuarios creados
